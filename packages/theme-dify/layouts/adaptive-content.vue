...
const slideTitle = computed(() => props.slideTitle || 'Adaptive Content')
...
<template>
  <div class="flex flex-col h-full px-[3rem] pt-[2rem] pb-[4rem] relative overflow-hidden bg-white">
    ...
    <div class="relative z-10 flex flex-col h-full">
      ...
      <div class="flex-grow min-h-0 relative z-10 w-full flex flex-col justify-center items-center">
        <div class="w-full max-w-4xl flex items-center justify-center">
          <slot>
            <BulletList v-if="items.length > 0" :items="items" size="lg" />
          </slot>
        </div>
      </div>
    </div>
